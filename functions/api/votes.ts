/**
 * GET    /api/votes  — return song IDs voted for today by this browser
 * POST   /api/votes  — cast a vote for a song
 * DELETE /api/votes  — retract a vote cast today
 *
 * Anonymous identity: a UUID is generated on first visit and stored in the
 * `sb_fp` cookie (1-year expiry). It is never linked to any personal data.
 *
...
function getFingerprint(request: Request): { fp: string; isNew: boolean } {
  ...
  const match  = cookie.match(/sb_fp=([a-f0-9-]{36})/);
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const [todayResult, everResult] = await Promise.all([
    env.DB.prepare(
      'SELECT song_id FROM votes WHERE fingerprint = ? AND vote_day = ?',
    ).bind(fp, todayUtc()).all(),
    env.DB.prepare(
      'SELECT DISTINCT song_id FROM votes WHERE fingerprint = ?',
    ).bind(fp).all(),
  ]);

  return Response.json({
    voted_today: todayResult.results.map((r: Record<string, unknown>) => r.song_id),
    voted_ever:  everResult.results.map((r: Record<string, unknown>) => r.song_id),
  });
};
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const { song_id } = body as Record<string, number>;
  if (!song_id || typeof song_id !== 'number') {
    return Response.json({ error: 'song_id (number) is required' }, { status: 400 });
  }
  ...
  // Insert vote — UNIQUE index prevents double-voting the same song on the same day.
  try {
    await env.DB.prepare(
      'INSERT INTO votes (song_id, fingerprint, vote_day) VALUES (?, ?, ?)',
    )
      .bind(song_id, fp, todayUtc())
      .run();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    if (msg.includes('UNIQUE')) {
      return Response.json({ error: 'Already voted for this song today' }, { status: 409 });
    }
    throw err;
  }
...
export const onRequestDelete: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const { results } = await env.DB.prepare(
    'SELECT id FROM votes WHERE song_id = ? AND fingerprint = ? AND vote_day = ?',
  )
    .bind(song_id, fp, todayUtc())
    .all();
  ...
  await env.DB.prepare(
    'DELETE FROM votes WHERE song_id = ? AND fingerprint = ? AND vote_day = ?',
  )
    .bind(song_id, fp, todayUtc())
    .run();
